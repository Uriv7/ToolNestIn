...
export default function StopwatchTimer() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2">{(['stopwatch','countdown'] as const).map(m=>(<button key={m} onClick={()=>{reset();setMode(m);}} className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border transition ${mode===m?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{m==='stopwatch'?'⏱ Stopwatch':'⏳ Countdown'}</button>))}</div>
      {mode==='countdown' && !running && (<div className="space-y-3"><div className="flex gap-2 flex-wrap">{PRESETS.map(p=>(<button key={p.l} onClick={()=>{setCdMin(p.m);setCdSec(p.s);setCdMs((parseInt(p.m)||0)*60000+(parseInt(p.s)||0)*1000);}} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/3 border border-white/10 text-slate-400 hover:text-slate-200 transition">{p.l}</button>))}</div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Minutes</label><input type="number" className="tool-input" value={cdMin} onChange={e => {setCdMin(e.target.value);setCdMs((parseInt(e.target.value)||0)*60000+(parseInt(cdSec)||0)*1000);}} aria-label="Minutes" /></div><div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Seconds</label><input type="number" className="tool-input" value={cdSec} onChange={e => {setCdSec(e.target.value);setCdMs((parseInt(cdMin)||0)*60000+(parseInt(e.target.value)||0)*1000);}} aria-label="Seconds" /></div></div></div>)}
      <div className="text-center"><div className={`font-mono font-extrabold text-6xl tracking-wider ${mode==='countdown'&&cdMs<10000?'text-red-400 animate-pulse':'gradient-text'}`}>{mode==='stopwatch'?fmt(ms):fmt(cdMs)}</div></div>
      <div className="flex gap-2 justify-center">
        {!running?<button onClick={start} className="px-8 py-3 rounded-xl bg-brand-500/20 border border-brand-400/40 text-brand-300 font-bold">▶ Start</button>:<button onClick={stop} className="px-8 py-3 rounded-xl bg-red-500/20 border border-red-400/40 text-red-300 font-bold">⏸ Pause</button>}
        <button onClick={reset} className="px-6 py-3 rounded-xl bg-white/3 border border-white/10 text-slate-400 font-bold">↺ Reset</button>
        {mode==='stopwatch'&&running&&<button onClick={()=>setLaps(l=>[...l,ms])} className="px-6 py-3 rounded-xl bg-white/3 border border-white/10 text-slate-400 font-bold">🏁 Lap</button>}
      </div>

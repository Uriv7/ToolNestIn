        ...
        <div className="flex gap-2">
          {(['v4', 'v7'] as const).map(v => (
            <button key={v} onClick={() => setVersion(v)}
              className={`px-4 py-2.5 rounded-xl text-sm font-bold border transition ${version === v ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
              UUID {v.toUpperCase()}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2 ml-4">
          ...
          {[1, 5, 10, 25].map(n => (
            <button key={n} onClick={() => setCount(n)}
              className={`w-9 h-8 rounded-lg text-xs font-bold border transition ${count === n ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500'}`}>
              {n}
            </button>
          ))}
        </div>

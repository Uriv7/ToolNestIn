  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Original</label><textarea className="tool-input h-40 font-mono text-xs" value={left} onChange={e=>setLeft(e.target.value)} placeholder="Paste first text here..." /></div>
        <div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Modified</label><textarea className="tool-input h-40 font-mono text-xs" value={right} onChange={e=>setRight(e.target.value)} placeholder="Paste second text here..." /></div>
      </div>
      {diffs.length > 0 && (

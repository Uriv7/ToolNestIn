  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2">
        {(['act','non-act'] as const).map(t => (
          <button key={t} onClick={() => setEmpType(t)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${empType===t?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Last Basic Salary (₹/mo)</label>
          <input type="number" className="tool-input" placeholder="50000" value={basic} onChange={e => setBasic(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">DA (₹/mo, if any)</label>
          <input type="number" className="tool-input" placeholder="0" value={da} onChange={e => setDa(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Years of Service</label>
          <input type="number" className="tool-input" placeholder="10" value={years} onChange={e => setYears(e.target.value)} min="5" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Months (in addition)</label>
      ...
      {show && (
        <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 space-y-3">
          ...
          <div className="flex justify-between border-t border-white/5 pt-3">
            <span className="text-slate-300 font-700">Gratuity Amount</span>
            <span className="gradient-text font-800 text-2xl">{fmt(gratuity)}</span>
          </div>
          <div className="flex justify-between text-sm"><span className="text-slate-400">Tax-Free (up to ₹20L)</span><span className="text-emerald-400 font-700">{fmt(Math.min(gratuity, taxFreeLimit))}</span></div>
          {taxable > 0 && <div className="flex justify-between text-sm"><span className="text-slate-400">Taxable Amount</span><span className="text-red-400 font-700">{fmt(taxable)}</span></div>}
        </div>
      )}

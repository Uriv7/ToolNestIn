...
export default function FDCalculator() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Quick bank rates */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Quick Select Bank Rate</label>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={r.l} className={`rounded-2xl p-4 border text-center ${r.h ? 'bg-brand-500/15 border-brand-400/40' : 'bg-white/3 border-white/10'}`}>
            <div className={`font-sans text-lg font-extrabold ${r.h ? 'gradient-text' : 'text-slate-200'}`}>{r.v}</div>

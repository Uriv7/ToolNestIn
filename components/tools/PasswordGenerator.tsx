...
export default function PasswordGenerator() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Length: {length}</label>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {[1, 3, 5, 10].map(n => (
            <button key={n} onClick={() => setCount(n)}
              className={`w-8 h-7 rounded-lg text-xs font-700 border transition ${count === n ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500'}`}>
              {n}
            </button>
          ))}
        </div>

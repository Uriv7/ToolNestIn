...
export default function CaseConverter() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Input Text</label>
      ...
      <div className="flex flex-wrap gap-2">
        {cases.map(c => (
          <button key={c.id} onClick={() => setActive(c.id)}
            className={`px-3 py-1.5 rounded-lg text-xs font-700 transition border ${active === c.id ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
            {c.label}
          </button>
        ))}
      </div>
      ...
      {output && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs font-600 text-slate-400 uppercase tracking-widest">Output — {current.label}</label>

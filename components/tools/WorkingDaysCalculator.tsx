    ...
    <div className="space-y-5">
      <div className="flex gap-2">
        {[false, true].map(m => (
          <button key={String(m)} onClick={() => setAddMode(m)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${addMode === m ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      {!addMode ? (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Start Date</label>
            <input type="date" className="tool-input" value={start} onChange={e => setStart(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">End Date</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">From Date</label>
            <input type="date" className="tool-input" value={start} onChange={e => setStart(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Add Working Days</label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.l} className={`rounded-2xl p-4 border text-center ${s.h ? 'bg-brand-500/15 border-brand-400/40' : 'bg-white/3 border-white/10'}`}>
              <div className={`font-display text-2xl font-800 ${s.h ? 'gradient-text' : 'text-slate-200'}`}>{s.v}</div>
      ...
      {addMode && addedDate && (
        <div className="bg-brand-500/10 border border-brand-400/30 rounded-2xl p-5 text-center">
          ...
          <div className="font-display text-2xl font-800 text-slate-100">{addedDate}</div>
        </div>
      )}

      ...
      <div className="flex gap-2">
        {(['lose', 'maintain', 'gain'] as const).map(g => (
          <button key={g} onClick={() => setGoal(g)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold border capitalize transition ${goal === g ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      {result && (
        <div className="space-y-4">
          <div className="bg-brand-500/10 border border-brand-400/30 rounded-2xl p-5 text-center">
            ...
            <div className="font-sans text-5xl font-black gradient-text">{Math.round(result.calories)}</div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={r.l} className="glass rounded-xl p-3 text-center">
                ...
                <div className="font-extrabold text-slate-100 text-lg">{r.v}</div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="glass rounded-xl p-3 text-center">
              ...
              <div className="font-bold text-slate-200">{Math.round(result.bmr)} kcal</div>
            </div>
            <div className="glass rounded-xl p-3 text-center">
              <div className="text-xs text-slate-500">TDEE (Total Daily Expenditure)</div>
              <div className="font-bold text-slate-200">{Math.round(result.tdee)} kcal</div>
            </div>
          </div>
        </div>
      )}

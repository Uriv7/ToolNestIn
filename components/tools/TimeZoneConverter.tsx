  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        {ZONES.slice(0,6).map(z=>(<div key={z.id} className="bg-white/3 border border-white/10 rounded-xl px-3 py-2 flex justify-between"><span className="text-slate-400">{z.l.split('—')[0]}</span><span className="text-slate-200 font-semibold">{now.toLocaleTimeString('en-IN',{timeZone:z.id,hour:'2-digit',minute:'2-digit',hour12:true})}</span></div>))}
      </div>
    </div>
  );
}

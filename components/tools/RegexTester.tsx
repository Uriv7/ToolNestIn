...
export default function RegexTester() {
  ...
  return (
    <div className="space-y-5">
      {/* Quick examples */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Quick Examples</label>
      ...
      {/* Pattern input */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Regular Expression</label>
      ...
      {/* Flags */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Flags</label>
        <div className="flex gap-2">
          {FLAGS.map(f => (
            <button key={f} onClick={() => toggleFlag(f)}
              className={`w-10 h-9 rounded-lg font-mono text-sm font-bold border transition ${flags.includes(f) ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
              {f}
            </button>
          ))}
      ...
      {/* Test string */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Test String</label>
          ...
          {!result.valid ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className={`text-sm font-semibold ${result.count > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>

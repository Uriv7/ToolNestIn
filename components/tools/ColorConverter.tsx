...
export default function ColorConverter() {
  ...
  const CopyRow = ({ label, value }: { label: string; value: string }) => (
    <div className="flex items-center justify-between px-4 py-3 bg-white/3 border border-white/8 rounded-xl">
      ...
      <code className="font-mono text-brand-300 font-semibold flex-1 mx-3">{value}</code>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-4 items-end">
        <div>
          <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Color Picker</label>
      ...
      {/* Presets */}
      <div>
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Presets</label>

...
export default function QRGenerator() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        {TYPES.map(t => (
          <button key={t.id} onClick={() => { setType(t.id); setQrDataUrl(''); }}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${type === t.id ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Size: {size}×{size}px</label>

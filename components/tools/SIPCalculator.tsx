    ...
    <div className="space-y-5">
      ...
      {result && (
        <>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={r.l} className={`rounded-2xl p-4 border text-center ${r.highlight ? 'bg-brand-500/15 border-brand-400/40' : 'bg-white/3 border-white/10'}`}>
                <div className={`font-sans text-xl font-extrabold ${r.highlight ? 'gradient-text' : 'text-slate-200'}`}>{r.v}</div>
          ...
          {/* Wealth growth chart */}
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Wealth Growth Over {years} Years</label>

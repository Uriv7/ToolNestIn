...
export default function TextToSlug() {
  ...
  return (
    <div className="space-y-4">
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Title or Text</label><input className="tool-input text-lg" value={input} onChange={e=>setInput(e.target.value)} placeholder="How to Calculate GST in India 2026" /></div>
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Domain Prefix (optional)</label><input className="tool-input text-sm" value={domain} onChange={e=>setDomain(e.target.value)} /></div>
      {slug && (<div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 space-y-3">
        <div><div className="text-xs text-slate-400 uppercase tracking-widest mb-1">Slug</div><div className="flex items-center gap-2"><code className="text-brand-300 font-800 text-lg font-mono">{slug}</code><button onClick={()=>navigator.clipboard.writeText(slug)} className="text-xs text-slate-500 hover:text-brand-400">📋</button></div></div>

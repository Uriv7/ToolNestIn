...
export default function TextToSlug() {
  ...
  return (
    <div className="space-y-4">
      <div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Title or Text</label><input className="tool-input text-lg" value={input} onChange={e => setInput(e.target.value)} aria-label="Title or Text" placeholder="How to Calculate GST in India 2026" /></div>
      <div><label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Domain Prefix (optional)</label><input className="tool-input text-sm" value={domain} onChange={e => setDomain(e.target.value)} aria-label="Domain Prefix (optional)" /></div>

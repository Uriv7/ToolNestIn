...
export default function GSTNumberValidator() {
  ...
  return (
    <div className="space-y-5">
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Enter GSTIN</label>
      ...
      {g.length===15 && (
        <div className={`rounded-2xl border p-5 space-y-3 ${isValid?'bg-gradient-to-br from-brand-950/60 to-dark-800 border-brand-400/20':'bg-red-500/10 border-red-400/30'}`}>
          <div className={`text-center font-800 text-lg ${isValid?'text-emerald-400':'text-red-400'}`}>{isValid?'✅ Valid GSTIN Format':'❌ Invalid GSTIN Format'}</div>
          {isValid && (<>
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[['State Code',stateCode],['State',state||'Unknown'],['Embedded PAN',pan],['Entity Type',entityType||entity]].map(([l,v])=>(<div key={l} className="bg-white/5 rounded-xl p-3"><div className="text-slate-400 text-xs mb-1">{l}</div><div className="text-slate-200 font-700">{v}</div></div>))}
            </div>

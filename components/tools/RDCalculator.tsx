      ...
      <div className="flex gap-2 flex-wrap">
        {PRESETS.map(p => (
          <button key={p.label} onClick={() => { setRate(p.rate); setMonths(p.months); }}
            className="px-3 py-1.5 rounded-lg text-xs font-600 bg-white/3 border border-white/10 text-slate-400 hover:text-slate-200 hover:border-white/20 transition">{p.label}</button>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Monthly Deposit (₹)</label>
        ...
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Tenure (months)</label>

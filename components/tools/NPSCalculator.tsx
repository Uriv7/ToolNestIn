  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Monthly Contribution (₹)</label>
          <input type="number" className="tool-input" value={monthly} onChange={e => setMonthly(e.target.value)} placeholder="5000" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Current Age</label>
          <input type="number" className="tool-input" value={age} onChange={e => setAge(e.target.value)} placeholder="30" min="18" max="59" />
        </div>
      </div>
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Equity Allocation: {equityPct}% (max 75% till age 50)</label>

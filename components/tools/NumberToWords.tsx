...
export default function NumberToWords() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Enter Number</label>
        <input type="number" className="tool-input text-xl font-700" value={input} onChange={e => setInput(e.target.value)} placeholder="e.g. 125000" min="0" />
      </div>

      <div className="flex gap-2">
        {(['INR', 'USD', 'plain'] as const).map(c => (
          <button key={c} onClick={() => setCurrency(c)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${currency === c ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      {result && (
        <div className="bg-brand-500/10 border border-brand-400/30 rounded-2xl p-5">
          <div className="font-display text-xl font-700 text-slate-100 leading-relaxed mb-4">{result}</div>

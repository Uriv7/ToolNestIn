...
export default function MarkdownToHTML() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">{(['edit','preview','html'] as const).map(t=>(<button key={t} onClick={()=>setTab(t)} className={`flex-1 py-2 rounded-xl text-sm font-semibold border transition ${tab===t?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{t==='edit'?'✏️ Markdown':t==='preview'?'👁 Preview':'<> HTML'}</button>))}</div>

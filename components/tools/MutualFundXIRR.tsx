  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-slate-400 uppercase tracking-widest px-1"><span>Date</span><span>Amount (₹, negative=invest, positive=redeem)</span></div>
      ...
      <button onClick={xirr} className="w-full py-2.5 rounded-xl bg-brand-500/20 border border-brand-400/40 text-brand-300 font-bold hover:bg-brand-500/30 transition">Calculate XIRR</button>
      {result !== null && (<div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 text-center">
        ...
        <div className={`font-extrabold text-4xl ${result>0?'gradient-text':'text-red-400'}`}>{result}% p.a.</div>

  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Pre-Pregnancy BMI</label><input type="number" className="tool-input" value={preBMI} onChange={e => setPreBMI(e.target.value)} placeholder="22" step="0.1" /></div>
        <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Current Week of Pregnancy</label><input type="number" className="tool-input" value={week} onChange={e => setWeek(e.target.value)} min="1" max="40" placeholder="20" /></div>
      </div>
      {range && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/3 border border-white/10 rounded-2xl p-4 text-center"><div className="gradient-text font-800 text-xl">{range[2]}–{range[3]} kg</div><div className="text-xs text-slate-500 mt-1">Weekly gain (2nd & 3rd trimester)</div></div>
            {w > 0 && <div className="bg-white/3 border border-white/10 rounded-2xl p-4 text-center"><div className="gradient-text font-800 text-xl">{expectedLow.toFixed(1)}–{expectedHigh.toFixed(1)} kg</div><div className="text-xs text-slate-500 mt-1">Expected gain at week {w}</div></div>}
          </div>

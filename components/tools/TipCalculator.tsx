...
export default function TipCalculator() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Bill Amount ($)</label>
        <input type="number" className="tool-input text-lg" value={bill} onChange={e => setBill(e.target.value)} placeholder="0.00" />
      </div>
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Tip Percentage</label>
        <div className="flex gap-2 flex-wrap">
          {TIPS.map(t => (
            <button key={t} onClick={() => setTip(t)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-700 transition border ${tip === t ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
              {t}%
            </button>
          ))}
        </div>
      ...
      <div>
        <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Number of People</label>
        <div className="flex items-center gap-3">
          ...
          <input type="number" className="tool-input text-center text-lg font-700" value={people} onChange={e => setPeople(e.target.value)} min="1" />
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={r.l} className={`rounded-2xl p-4 border text-center ${r.highlight ? 'bg-brand-500/15 border-brand-400/40' : 'bg-white/3 border-white/10'}`}>
              <div className={`font-display text-2xl font-800 ${r.highlight ? 'gradient-text' : 'text-slate-200'}`}>{r.v}</div>

  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {['all','2xx','3xx','4xx','5xx'].map(f=>(<button key={f} onClick={()=>setFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition ${filter===f?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{f}</button>))}
      ...
      <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
        {filtered.map(c=>(<div key={c.c} className="flex gap-3 bg-white/3 border border-white/10 rounded-xl p-3 hover:border-white/20 transition">
          <span className={`font-extrabold text-lg font-mono w-12 flex-shrink-0 ${CAT_COLORS[c.cat]}`}>{c.c}</span>
          <div><div className="text-slate-200 font-semibold text-sm">{c.l}</div><div className="text-slate-400 text-xs mt-0.5">{c.d}</div></div>
        </div>))}
      </div>
    </div>
  );

  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Basic Salary (₹/mo)</label>
        ...
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">HRA Received (₹/mo)</label>
          <input type="number" className="tool-input" placeholder="20000" value={hraReceived} onChange={e => setHraReceived(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Rent Paid (₹/mo)</label>
      ...
      <div className="flex gap-2">
        {[true, false].map(m => (
          <button key={String(m)} onClick={() => setIsMetro(m)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${isMetro === m ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      {show && (
        <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 space-y-3">
          <p className="text-xs font-600 text-slate-400 uppercase tracking-widest mb-1">HRA Exemption — Minimum of 3 criteria</p>
          {[
            ...
            <div key={row.label} className={`flex justify-between items-center py-2 border-b border-white/5 ${row.winner ? 'text-brand-300' : 'text-slate-400'}`}>
              ...
              <span className={`font-700 ${row.winner ? 'gradient-text text-lg' : ''}`}>{fmt(row.value)}</span>
            </div>
          ))}
          <div className="flex justify-between pt-2">
            <span className="text-slate-300 font-700">HRA Exemption</span>
            <span className="gradient-text font-800 text-xl">{fmt(exemption)}/mo</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Taxable HRA</span>
            <span className="text-red-400 font-700">{fmt(taxable)}/mo</span>
          </div>
          <div className="flex justify-between text-sm border-t border-white/5 pt-2">
            <span className="text-slate-400">Annual HRA Exemption</span>
            <span className="text-emerald-400 font-700">{fmt(exemption * 12)}/year</span>
          </div>
        </div>
      )}

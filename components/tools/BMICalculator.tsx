      ...
      <div className="flex gap-2">
        {(['metric', 'imperial'] as const).map(u => (
          <button key={u} onClick={() => setUnit(u)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 transition border ${unit === u ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {unit === 'metric' ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Feet</label>
              <input type="number" className="tool-input" value={height} onChange={e => setHeight(e.target.value)} placeholder="5" />
            </div>
            <div>
              <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Inches</label>
      ...
      {bmi > 0 && cat && (
        <div className="bg-gradient-to-br from-dark-700 to-dark-800 rounded-2xl p-6 border border-white/10">
          <div className="text-center mb-5">
            <div className="font-display text-6xl font-900 mb-2" style={{ color: cat.color }}>
              {bmi.toFixed(1)}
            </div>
            <div className="text-lg font-600" style={{ color: cat.color }}>{cat.label}</div>
          </div>

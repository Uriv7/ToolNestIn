...
export default function EmojiPicker() {
  ...
  return (
    <div className="space-y-4">
      ...
      {!q && (<div className="flex gap-2 flex-wrap">{Object.keys(EMOJIS).map(c=>(<button key={c} onClick={()=>setCat(c)} className={`px-3 py-1.5 rounded-lg text-xs font-600 border transition ${cat===c?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{c}</button>))}</div>)}

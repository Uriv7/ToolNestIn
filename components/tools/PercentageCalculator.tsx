  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        {modes.map(m => (
          <button key={m.id} onClick={() => setMode(m.id)}
            className={`px-4 py-2 rounded-xl text-sm font-600 transition border ${mode === m.id ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
            {m.label}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">{labels[mode].a}</label>
          <input type="number" className="tool-input" value={a} onChange={e => setA(e.target.value)} placeholder="e.g. 20" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">{labels[mode].b}</label>
      ...
      {result && (
        <div className="bg-brand-500/10 border border-brand-400/30 rounded-2xl p-6 text-center">
          ...
          <div className="font-display text-4xl font-900 gradient-text">{result}</div>
        </div>
      )}
    </div>
  );

  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Annual Deposit (₹)</label><input type="number" className="tool-input" value={annual} onChange={e => setAnnual(e.target.value)} min="250" max="150000" /></div>
        <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Girl's Current Age (years)</label><input type="number" className="tool-input" value={girlAge} onChange={e => setGirlAge(e.target.value)} min="0" max="9" /></div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[{ l: 'Total Deposited', v: fmt(result.invested) }, { l: 'Interest Earned', v: fmt(result.interest) }, { l: 'Maturity Amount', v: fmt(result.corpus) }].map(r => (
          <div key={r.l} className="bg-white/3 border border-white/10 rounded-2xl p-4 text-center">
            <div className="font-display text-lg font-800 gradient-text">{r.v}</div>

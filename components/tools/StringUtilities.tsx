...
export default function StringUtilities() {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{modes.map(([m,l])=>(<button key={m} onClick={()=>setMode(m)} className={`py-2 px-2 rounded-xl text-xs font-600 border transition ${mode===m?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{l}</button>))}</div>
      {mode==='replace' && (<div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Find</label><input className="tool-input" value={find} onChange={e=>setFind(e.target.value)} placeholder="text to find" /></div><div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Replace with</label><input className="tool-input" value={replace} onChange={e=>setReplace(e.target.value)} placeholder="replacement" /></div></div>)}

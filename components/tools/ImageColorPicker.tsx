...
export default function ImageColorPicker() {
  ...
  return (
    <div className="space-y-4">
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Upload Image</label><input type="file" accept="image/*" onChange={handleFile} className="tool-input text-sm" /></div>
      ...
      {picked && rgb && (
        <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 flex items-center gap-5">
          ...
          <div className="space-y-1">
            <div className="flex gap-2 items-center"><span className="font-mono font-800 text-xl text-slate-200">{picked.toUpperCase()}</span><button onClick={()=>navigator.clipboard.writeText(picked.toUpperCase())} className="text-xs text-slate-500 hover:text-brand-400">📋</button></div>

...
export default function WaterIntakeCalculator() {
  ...
  return (
    <div className="space-y-5">
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Body Weight (kg)</label><input type="number" className="tool-input" value={weight} onChange={e => setWeight(e.target.value)} placeholder="70" /></div>
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Activity Level</label>
        <div className="grid grid-cols-2 gap-2">{levels.map(l=>(<button key={l.id} onClick={()=>setActivity(l.id)} className={`py-2 rounded-xl text-sm font-600 border transition ${activity===l.id?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{l.label}</button>))}</div>
      </div>
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Climate</label>
        <div className="grid grid-cols-3 gap-2">{[{id:'normal',l:'Normal'},{id:'hot',l:'Hot Summer'},{id:'very-hot',l:'Very Hot 40°C+'}].map(c=>(<button key={c.id} onClick={()=>setClimate(c.id)} className={`py-2 rounded-xl text-sm font-600 border transition ${climate===c.id?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{c.l}</button>))}</div>
      </div>
      {w > 0 && (
        <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 text-center space-y-2">
          <div className="gradient-text font-800 text-4xl">{(total/1000).toFixed(1)} L</div>

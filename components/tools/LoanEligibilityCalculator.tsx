      ...
      {ni > 0 && loanAmount > 0 && (
        <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 space-y-3">
          <div className="text-center">
            ...
            <div className="gradient-text font-extrabold text-4xl">{fmt(loanAmount)}</div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-white/5 text-sm">
            <div><span className="text-slate-400">Max EMI capacity</span><div className="text-slate-200 font-bold">{fmt(maxEmi)}/mo</div></div>
            <div><span className="text-slate-400">Available after existing EMIs</span><div className="text-slate-200 font-bold">{fmt(maxEmi)}/mo</div></div>
          </div>
        </div>
      )}

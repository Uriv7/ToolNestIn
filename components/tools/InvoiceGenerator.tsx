  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2"><div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Your Business</div>
        ...
        <div className="space-y-2"><div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Bill To</div>
      ...
      <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-4 space-y-2 text-sm">
        ...
        <div className="flex justify-between font-extrabold text-slate-200 border-t border-white/10 pt-2"><span>Total Amount</span><span className="gradient-text text-xl">{fmt(totals.total)}</span></div>
      </div>
      <button onClick={print} className="w-full py-2.5 rounded-xl bg-brand-500/20 border border-brand-400/40 text-brand-300 font-bold hover:bg-brand-500/30 transition">🖨️ Print / Save as PDF</button>

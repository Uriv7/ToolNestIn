    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        {Object.entries(UNITS).map(([k, v]) => (
          <button key={k} onClick={() => { setCategory(k); setFrom(v.units[0].id); }}
            className={`px-4 py-2 rounded-xl text-sm font-600 border transition ${category === k ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
            {v.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Value</label>
          <input type="number" className="tool-input text-lg" value={value} onChange={e => setValue(e.target.value)} />
        </div>
        <div className="flex-1">
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">From Unit</label>
      ...
      <div className="space-y-2">
        {units.filter(u => u.id !== from).map(u => (
          <div key={u.id} className="flex items-center justify-between px-4 py-3 bg-white/3 border border-white/8 rounded-xl hover:bg-white/5 transition">
            ...
            <span className="font-mono text-brand-300 font-600">{convert(u.id)}</span>
          </div>
        ))}
      </div>
    </div>

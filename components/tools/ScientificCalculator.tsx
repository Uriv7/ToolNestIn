...
export default function ScientificCalculator() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center"><label className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Mode</label><button onClick={()=>setIsDeg(d=>!d)} className="px-3 py-1 rounded-lg text-xs font-bold bg-white/3 border border-white/10 text-slate-300">{isDeg?'DEG':'RAD'}</button></div>
      <div className="bg-dark-900 rounded-2xl border border-white/10 p-4 text-right"><div className="font-mono font-extrabold text-3xl text-slate-200 break-all">{display}</div>{memory!==0&&<div className="text-xs text-slate-500 mt-1">M: {memory}</div>}</div>

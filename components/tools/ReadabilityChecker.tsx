...
export default function ReadabilityChecker() {
  ...
  const analyse = () => {
    ...
    // SMOG Index — standard formula, calibrated for 30-sentence samples
    const smog = Math.round(1.043 * Math.sqrt(complexWords * (30 / sc)) + 3.1291);
    // Coleman-Liau Index — uses letters/sentences per 100 words instead of syllables
    const L = (letters / wc) * 100;
    const S = (sc / wc) * 100;
    const coleman = Math.round(0.0588 * L - 0.296 * S - 15.8);
    const getLabel = (f: number) => f>=90?'Very Easy':f>=70?'Easy':f>=60?'Standard':f>=50?'Fairly Difficult':f>=30?'Difficult':'Very Difficult';
    return { flesch, fkgl, fog, smog, coleman, wc, sc, syllables, readTime: Math.ceil(wc/200), label: getLabel(flesch) };
  };
    ...
    <div className="space-y-5">
      ...
      {r && (<div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[{l:'Grade Level',v:`Grade ${Math.max(1,r.fkgl)}`},{l:'Gunning Fog',v:r.fog+' grade'},{l:'SMOG Index',v:'Grade '+Math.max(1,r.smog)},{l:'Coleman-Liau',v:'Grade '+Math.max(1,r.coleman)},{l:'Read Time',v:r.readTime+' min'}].map(s=>(<div key={s.l} className="bg-white/3 border border-white/10 rounded-xl p-3 text-center"><div className="text-slate-200 font-extrabold">{s.v}</div><div className="text-xs text-slate-500 mt-1">{s.l}</div></div>))}
        </div>

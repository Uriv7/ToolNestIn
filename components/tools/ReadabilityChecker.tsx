...
export default function ReadabilityChecker() {
  ...
  return (
    <div className="space-y-5">
      <div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Paste Your Content</label><textarea className="tool-input h-40" value={text} onChange={e=>setText(e.target.value)} placeholder="Paste your blog post, article, or any text here (minimum 50 words for accurate results)..." /></div>
      {r && (<div className="space-y-3">
        <div className={`bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 text-center`}>
          ...
          <div className={`font-800 text-5xl ${color}`}>{Math.min(100,Math.max(0,r.flesch))}</div>
          <div className={`font-700 mt-1 ${color}`}>{r.label}</div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {[{l:'Grade Level',v:`Grade ${Math.max(1,r.fkgl)}`},{l:'Gunning Fog',v:r.fog+' grade'},{l:'Read Time',v:r.readTime+' min'}].map(s=>(<div key={s.l} className="bg-white/3 border border-white/10 rounded-xl p-3 text-center"><div className="text-slate-200 font-800">{s.v}</div><div className="text-xs text-slate-500 mt-1">{s.l}</div></div>))}
        </div>
        <div className="grid grid-cols-3 gap-3 text-xs">
          {[{l:'Words',v:r.wc},{l:'Sentences',v:r.sc},{l:'Syllables',v:r.syllables}].map(s=>(<div key={s.l} className="bg-white/3 border border-white/10 rounded-xl px-3 py-2 flex justify-between"><span className="text-slate-400">{s.l}</span><span className="text-slate-200 font-700">{s.v}</span></div>))}
        </div>

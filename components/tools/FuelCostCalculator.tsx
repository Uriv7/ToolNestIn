  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {(['petrol', 'diesel', 'cng'] as const).map(f => (
          <button key={f} onClick={() => { setFuelType(f); setFuelPrice(FUEL_PRICES[f].price); }}
            className={`flex-1 py-2.5 rounded-xl text-sm font-600 border capitalize transition ${fuelType === f ? 'bg-brand-500/20 border-brand-400/50 text-brand-300' : 'bg-white/3 border-white/10 text-slate-500 hover:text-slate-300'}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Distance (km)</label>
          <input type="number" className="tool-input" value={distance} onChange={e => setDistance(e.target.value)} placeholder="30" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Mileage (km/l)</label>
          <input type="number" className="tool-input" value={mileage} onChange={e => setMileage(e.target.value)} placeholder="15" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">{FUEL_PRICES[fuelType].label}</label>
          <input type="number" className="tool-input" value={fuelPrice} onChange={e => setFuelPrice(e.target.value)} />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Trips per Month</label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={r.l} className={`rounded-2xl p-4 border text-center ${r.h ? 'bg-brand-500/15 border-brand-400/40' : 'bg-white/3 border-white/10'}`}>
              <div className={`font-display text-xl font-800 ${r.h ? 'gradient-text' : 'text-slate-200'}`}>{r.v}</div>

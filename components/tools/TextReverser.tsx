...
export default function TextReverser() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">{([['chars','🔤 Reverse Characters'],['words','📝 Reverse Words'],['lines','📋 Reverse Lines']] as const).map(([m,l])=>(<button key={m} onClick={()=>setMode(m)} className={`flex-1 py-2.5 rounded-xl text-sm font-600 border transition ${mode===m?'bg-brand-500/20 border-brand-400/50 text-brand-300':'bg-white/3 border-white/10 text-slate-500'}`}>{l}</button>))}</div>
      <textarea className="tool-input h-32" value={input} onChange={e=>setInput(e.target.value)} placeholder="Type or paste text here..." />
      {input && (<div><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Result</label>

    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="sm:col-span-1">
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Annual CTC (₹)</label>
          <input type="number" className="tool-input text-lg font-700" value={ctc} onChange={e => setCtc(e.target.value)} placeholder="1200000" />
        </div>
        <div>
          <label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">Tax Regime</label>
      ...
      <div className="bg-brand-500/10 border border-brand-400/30 rounded-2xl p-5 text-center">
        ...
        <div className="font-display text-5xl font-900 gradient-text">{fmt(result.monthlyInHand)}</div>
        <div className="text-sm text-slate-500 mt-1">Annual: {fmt(result.annualInHand)}</div>
      </div>

      {/* Breakup */}
      <div className="space-y-2">
        <h3 className="text-xs font-600 text-slate-400 uppercase tracking-widest">Monthly Salary Breakup</h3>
        {[
          ...
          <div key={r.l} className="flex justify-between px-4 py-2.5 bg-white/3 border border-white/8 rounded-xl">
            ...
            <span className={`font-700 text-sm ${r.color}`}>{r.v}</span>
          </div>
        ))}
      </div>

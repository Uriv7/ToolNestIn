...
export default function CronGenerator() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">{PRESETS.map(p=>(<button key={p.l} onClick={()=>apply(p.v)} className="px-3 py-1.5 rounded-lg text-xs font-600 bg-white/3 border border-white/10 text-slate-400 hover:text-slate-200 hover:border-white/20 transition">{p.l}</button>))}</div>
      <div className="grid grid-cols-5 gap-2">
        {[['Minute','0-59',min,setMin],['Hour','0-23',hour,setHour],['Day','1-31',dom,setDom],['Month','1-12',month,setMonth],['Weekday','0-7',dow,setDow]].map(([l,p,v,s]:any)=>(
          <div key={l}><label className="block text-xs font-600 text-slate-400 uppercase tracking-widest mb-2">{l}</label><input className="tool-input text-center" value={v} onChange={e=>s(e.target.value)} placeholder={p} /></div>
        ))}
      </div>
      <div className="bg-gradient-to-br from-brand-950/60 to-dark-800 rounded-2xl border border-brand-400/20 p-5 space-y-3">
        <div className="flex items-center gap-3"><code className="text-brand-300 font-800 text-xl font-mono">{expr}</code><button onClick={()=>navigator.clipboard.writeText(expr)} className="text-xs text-slate-500 hover:text-brand-400">📋 Copy</button></div>

...
export default function BlogArticle({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16">
      ...
      <div className="mb-8">
        <span className="text-xs font-600 px-2.5 py-1 rounded-full bg-brand-500/15 text-brand-300 mr-3">{article.tag}</span>
        <span className="text-slate-600 text-xs">{article.date} · {article.readTime} read</span>
        <h1 className="font-display text-3xl sm:text-4xl font-800 text-slate-100 mt-4 mb-4 leading-tight">{article.title}</h1>
      ...
      {/* Article body */}
      <article className="text-slate-400 leading-relaxed space-y-4 [&_h2]:font-display [&_h2]:text-xl [&_h2]:font-700 [&_h2]:text-slate-100 [&_h2]:mt-8 [&_h2]:mb-3 [&_h3]:font-600 [&_h3]:text-slate-200 [&_h3]:mt-5 [&_h3]:mb-2 [&_strong]:text-slate-300 [&_a]:text-brand-400 [&_a:hover]:underline [&_ul]:space-y-2 [&_ul]:pl-5 [&_ul]:list-disc [&_ol]:space-y-2 [&_ol]:pl-5 [&_ol]:list-decimal [&_p]:text-sm [&_li]:text-sm">
        {content}
      </article>
      ...
      <section className="glass rounded-2xl p-6">
        <h2 className="font-display font-700 text-slate-200 mb-4">More guides</h2>
        <div className="space-y-3">
          {ARTICLES.filter(a => a.slug !== params.slug).slice(0, 3).map(a => (
            <Link key={a.slug} href={`/blog/${a.slug}`}
              ...
              <div>
                <div className="text-sm font-600 text-slate-300 hover:text-brand-300">{a.title}</div>

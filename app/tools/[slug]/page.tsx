        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="glass rounded-2xl p-6 mb-5">
            <div className="flex items-start gap-4 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="font-display text-2xl sm:text-3xl font-800 text-slate-100 mb-1 leading-tight">{h1}</h1>
          ...
          <div className="flex items-center gap-3 mb-5 p-4 glass rounded-2xl border border-white/5 flex-wrap">
            ...
            <a href={`https://wa.me/?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/15 border border-emerald-400/30 text-emerald-400 text-sm font-600 hover:bg-emerald-500/25 transition">
              📲 WhatsApp
            </a>
            <a href={`https://twitter.com/intent/tweet?text=${twMsg}`} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-sky-500/15 border border-sky-400/30 text-sky-400 text-sm font-600 hover:bg-sky-500/25 transition">
              𝕏 Tweet
            </a>
          </div>
          ...
          <section className="glass rounded-2xl p-6 mb-5">
            <h2 className="font-display text-xl font-700 text-slate-100 mb-4">
            ...
            <h3 className="font-display text-base font-700 text-slate-200 mt-5 mb-3">How to use</h3>
            <ol className="space-y-2" role="list">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-sm text-slate-400">
                  <span className="w-5 h-5 rounded-full bg-brand-500/20 border border-brand-400/30 text-brand-400 text-xs font-700 flex items-center justify-center shrink-0 mt-0.5" aria-hidden="true">{i + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
          ...
          <section className="glass rounded-2xl p-5 mb-5">
            <h2 className="font-display text-base font-700 text-slate-200 mb-4">Tool details</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={f.label} className="bg-white/3 rounded-xl p-3 text-center border border-white/8">
                  ...
                  <div className="text-sm font-600 text-slate-300 mt-0.5">{f.value}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          {tool.workflow.length > 0 && (
            <section className="glass rounded-2xl p-5 mb-5 border border-brand-400/20">
              <h2 className="font-display font-700 text-slate-100 text-base mb-4">⚡ Next step — complete your workflow</h2>
                ...
                {tool.workflow.map(w => {
                  ...
                  return (
                    <Link key={w.slug} href={`/tools/${w.slug}`}
                      ...
                      <div>
                        <div className="font-600 text-brand-300 text-sm group-hover:text-brand-200 transition">{w.label} →</div>
          ...
          {tool.expertNote && (
            <section className="rounded-2xl p-5 mb-5 border border-amber-400/25 bg-amber-500/5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="font-display font-700 text-amber-300 text-xs uppercase tracking-widest mb-2">
          ...
          {tool.faqs.length > 0 && (
            <section className="glass rounded-2xl p-6 mb-5">
              <h2 className="font-display text-xl font-700 text-slate-100 mb-5">Frequently asked questions</h2>
              <div className="space-y-4">
                {tool.faqs.map((faq, i) => (
                  <div key={i} className="border-b border-white/5 pb-4 last:border-0 last:pb-0">
                    <h3 className="font-600 text-slate-200 mb-2 text-sm">{faq.q}</h3>
          ...
          {related.length > 0 && (
            <section className="glass rounded-2xl p-5 mb-5">
              <h2 className="font-display text-base font-700 text-slate-200 mb-4">More {tool.category} tools</h2>
                ...
                {related.map(r => (
                  <Link key={r.slug} href={`/tools/${r.slug}`}
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-600 text-slate-300 truncate">{r.name.split('—')[0].trim()}</div>
          ...
          <section className="glass rounded-2xl p-5">
            <h2 className="font-display text-base font-700 text-slate-200 mb-4">Explore more free tools</h2>
        ...
        <aside className="hidden lg:block w-64 shrink-0" aria-label="Related tools">
          <div className="sticky top-24 space-y-4">
            ...
            <div className="glass rounded-2xl p-5">
              <h3 className="text-xs font-700 text-slate-400 uppercase tracking-widest mb-4">Tool info</h3>
              <div className="space-y-2 text-sm">
                {[['Category', tool.category],['Processing', 'Browser-only'],
                  ['Data sent', 'None'],['Login needed', 'No'],['Cost', 'Free']].map(([k, v]) => (
                  <div key={k} className="flex justify-between">
                    <span className="text-slate-500">{k}</span>
                    <span className="text-slate-300 font-600">{v}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {related.length > 0 && (
              <div className="glass rounded-2xl p-5">
                <h3 className="text-xs font-700 text-slate-400 uppercase tracking-widest mb-4">Related tools</h3>
            ...
            <div className="glass rounded-2xl p-5">
              <h3 className="text-xs font-700 text-slate-400 uppercase tracking-widest mb-3">Share</h3>
              <div className="space-y-2">
                <a href={`https://wa.me/?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 w-full px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-400/20 text-emerald-400 text-xs font-600 hover:bg-emerald-500/20 transition">
                  📲 Share on WhatsApp
                </a>
                <a href={`https://twitter.com/intent/tweet?text=${twMsg}`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-2 w-full px-3 py-2 rounded-lg bg-sky-500/10 border border-sky-400/20 text-sky-400 text-xs font-600 hover:bg-sky-500/20 transition">
                  𝕏 Share on Twitter
                </a>
              </div>
            </div>
            ...
            <div className="glass rounded-2xl p-5">
              <h3 className="text-xs font-700 text-slate-400 uppercase tracking-widest mb-3">Learn more</h3>
